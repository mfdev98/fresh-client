...
import Review from '../../libs/components/booking/BookingReview';
import { Swiper, SwiperSlide } from 'swiper/react';
import SwiperCore, { Autoplay, Navigation, Pagination } from 'swiper';
import BookingBigCard from '../../libs/components/common/BookingBigCard';
...
import { GET_BOOKINGS, GET_BOOKING } from '../../apollo/user/query';
import { T } from '../../libs/types/common';
import { Direction, Message } from '../../libs/enums/common.enum';
import { CREATE_COMMENT, LIKE_TARGET_BOOKING } from '../../apollo/user/mutation';
...
const BookingDetail: NextPage = ({ initialComment, ...props }: any) => {
	...
	const [bookingComments, setBookingComments] = useState<Comment[]>([]);
	const [commentTotal, setCommentTotal] = useState<number>(0);
	const [insertCommentData, setInsertCommentData] = useState<CommentInput>({
		commentGroup: CommentGroup.BOOKING,
		commentContent: '',
		commentRefId: '',
	});
	...
	const {
		...
		onCompleted: (data: T) => {
			if (data?.getBooking) setBooking(data?.getBooking);
			if (data?.getBooking) setSlideImage(data?.getBooking?.bookingImages[0]);
		},
	});
	...
	const {
		...
		variables: {
			input: {
				...
				search: {
					locationList: booking?.bookingLocation ? [booking?.bookingLocation] : [],
				},
			},
		},
		skip: !bookingId && !booking,
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getBooking?.list) setDestinationBookings(data?.getBookings?.list);
		},
	});
	...
	const {
		...
		notifyOnNetworkStatusChange: true,
		onCompleted: (data: T) => {
			if (data?.getComments?.list) setBookingComments(data?.getComments?.list);
	...
	useEffect(() => {
		if (router.query.id) {
			setBookingId(router.query.id as string);
			setCommentInquiry({
				...commentInquiry,
				search: {
	...
	const likeBookingHandler = async (user: T, id: string) => {
		try {
			...
			await getBookingRefetch({ input: id });
			await getBookingRefetch({
				input: {
					page: 1,
					limit: 4,
					sort: 'createdAt',
					direction: Direction.DESC,
					search: {
						locationList: [booking?.bookingLocation],
					},
				},
			});
					...
					<Stack className={'property-info-config'}>
						<Stack className={'info'}>
							<Stack className={'left-box'}>
								<Typography className={'title-main'}>{booking?.bookingTitle}</Typography>
								<Stack className={'top-box'}>
									<Typography className={'city'}>{booking?.bookingLocation}</Typography>
									...
									<Typography className={'date'}>{moment().diff(booking?.createdAt, 'days')} days ago</Typography>
								</Stack>
							</Stack>
							<Stack className={'right-box'}>
								<Stack className="buttons">
									<Stack className="button-box">
										...
										<Typography>{booking?.bookingViews}</Typography>
									</Stack>
									<Stack className="button-box">
										{booking?.meLiked && booking?.meLiked[0]?.myFavorite ? (
											...
											<FavoriteBorderIcon
												fontSize={'medium'}
												// @ts-ignore
												onClick={() => likeBookingHandler(user, booking?._id)}
											/>
										)}
										<Typography>{booking?.bookingLikes}</Typography>
									</Stack>
								</Stack>
						...
						<Stack className={'images'}>
							<Stack className={'main-image'}>
								<img
									src={slideImage ? `${REACT_APP_API_URL}/${slideImage}` : '/img/booking/bigImage.png'}
									alt={'main-image'}
								/>
							</Stack>
							<Stack className={'sub-images'}>
								{booking?.bookingImages.map((subImg: string) => {
					...
					<Stack className={'property-desc-config'}>
						<Stack className={'left-config'}>
							<Stack className={'prop-desc-config'}>
								<Stack className={'top'}>
									<Typography className={'title'}>Booking Description</Typography>
									<Typography className={'desc'}>{booking?.bookingDesc ?? 'No Description!'}</Typography>
								</Stack>
									...
									<Stack className={'info-box'}>
										<Stack className={'left'}>
											<Box component={'div'} className={'info'}>
												...
												<Typography className={'data'}>${formatterStr(booking?.bookingPrice)}</Typography>
											</Box>
										</Stack>
										<Stack className={'right'}>
											<Box component={'div'} className={'info'}>
												<Typography className={'title'}>Booking Type</Typography>
												<Typography className={'data'}>{booking?.bookingType}</Typography>
											</Box>
										</Stack>
									</Stack>
								</Stack>
							...
							{commentTotal !== 0 && (
								<Stack className={'reviews-config'}>
									...
									<Stack className={'review-list'}>
										{bookingComments?.map((comment: Comment) => {
							...
							<Stack className={'info-box'}>
								...
								<Stack className={'image-info'}>
									<img
										...
										src={
											booking?.memberData?.memberImage
												? `${REACT_APP_API_URL}/${booking?.memberData?.memberImage}`
												: '/img/profile/defaultUser.svg'
										}
									/>
									<Stack className={'name-phone-listings'}>
										<Link href={`/member?memberId=${booking?.memberData?._id}`}>
											<Typography className={'name'}>{booking?.memberData?.memberNick}</Typography>
										</Link>
										<Stack className={'phone-number'}>
											...
											<Typography className={'number'}>{booking?.memberData?.memberPhone}</Typography>
										</Stack>
						...
						<Stack className={'similar-properties-config'}>
							<Stack className={'title-pagination-box'}>
								<Stack className={'title-box'}>
									<Typography className={'main-title'}>Destination Booking</Typography>
							...
							<Stack className={'cards-box'}>
								...
								>
									{destinationBookings.map((booking: Booking) => {
										return (
											<SwiperSlide className={'similar-homes-slide'} key={booking.bookingTitle}>
												<BookingBigCard booking={booking} likeBookingHandler={likeBookingHandler} key={booking?._id} />
											</SwiperSlide>
										);
									})}
								</Swiper>
...
export default withLayoutFull(BookingDetail);
