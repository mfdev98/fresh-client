...
import { CREATE_BOOKING, UPDATE_BOOKING } from '../../../apollo/user/mutation';
import { GET_BOOKING } from '../../../apollo/user/query';

const AddBooking = ({ initialValues, ...props }: any) => {
	const device = useDeviceDetect();
	const router = useRouter();
	const inputRef = useRef<any>(null);
	const [insertBookingData, setInsertBookingData] = useState<BookingInput>(initialValues);
	...
	const {
		...
		refetch: getBookingRefetch,
	} = useQuery(GET_BOOKING, {
		fetchPolicy: 'network-only',
		variables: {
			input: router.query.bookingId,
		},
	});

	/** LIFECYCLES **/
	useEffect(() => {
		setInsertBookingData({
			...insertBookingData,
			bookingTitle: getBookingData?.getBooking ? getBookingData?.getBooking?.bookingTitle : '',
			bookingPrice: getBookingData?.getBooking ? getBookingData?.getBooking?.bookingPrice : 0,
			bookingType: getBookingData?.getBooking ? getBookingData?.getBooking?.bookingType : '',
			bookingLocation: getBookingData?.getBooking ? getBookingData?.getBooking?.bookingLocation : '',
			bookingDesc: getBookingData?.getBooking ? getBookingData?.getBooking?.bookingDesc : '',
			bookingImages: getBookingData?.getBooking ? getBookingData?.getBooking?.bookingImages : [],
		});
	}, [getBookingLoading, getBookingData]);
	...
	async function uploadImages() {
		try {
			...
			setInsertBookingData({ ...insertBookingData, bookingImages: responseImages });
		} catch (err: any) {
	...
	const doDisabledCheck = () => {
		if (
			insertBookingData.bookingTitle === '' ||
			insertBookingData.bookingPrice === 0 || // @ts-ignore
			insertBookingData.bookingType === '' || // @ts-ignore
			insertBookingData.bookingLocation === '' || // @ts-ignore
			insertBookingData.bookingDesc === '' ||
			insertBookingData.bookingImages.length === 0
		) {
			return true;
		}
	};

	const insertBookingHandler = useCallback(async () => {
		try {
			const result = await createBooking({
				variables: {
					input: insertBookingData,
				},
			});

			await sweetMixinSuccessAlert('This booking has been created successfully!');
			await router.push({
				pathname: '/mypage',
				query: {
	...
	const updateBookingHandler = useCallback(async () => {
		try {
			//@ts-ignore
			insertBookingData._id = getBookingData?.getBooking?._id;
			const result = await updateBooking({
				variables: {
					input: insertBookingData,
				},
			});
			await sweetMixinSuccessAlert('This Booking has been updated successfully!');
			await router.push({
				pathname: '/mypage',
				query: {
					category: 'myProperties',
	...
	console.log('+insertBookingData', insertBookingData);

	return (
		<div id="add-property-page">
			<Stack className="main-title-box">
				<Typography className="main-title">Add New Booking</Typography>
				...
				<Stack className="config">
					<Stack className="description-box">
						<Stack className="config-column">
							...
							<input
								...
								placeholder={'Title'}
								value={insertBookingData.bookingTitle}
								onChange={({ target: { value } }) =>
									setInsertBookingData({ ...insertBookingData, bookingTitle: value })
								}
							/>
						</Stack>
						...
						<Stack className="config-row">
							<Stack className="price-year-after-price">
								...
								<input
									...
									className="description-input"
									placeholder={'Price'}
									value={insertBookingData.bookingPrice}
									onChange={({ target: { value } }) =>
										setInsertBookingData({ ...insertBookingData, bookingPrice: parseInt(value) })
									}
								/>
							</Stack>
							<Stack className="price-year-after-price">
								...
								<select
									...
									defaultValue={insertBookingData.bookingType || 'select'}
									value={insertBookingData.bookingType || 'select'}
									onChange={({ target: { value } }) =>
										// @ts-ignore
										setInsertBookingData({ ...insertBookingData, bookingType: value })
									}
								>
									<>
						...
						<Stack className="config-row">
							<Stack className="price-year-after-price">
								...
								<select
									className={'select-description'}
									defaultValue={insertBookingData.bookingLocation || 'select'}
									value={insertBookingData.bookingLocation || 'select'}
									onChange={({ target: { value } }) =>
										// @ts-ignore
										setInsertBookingData({ ...insertBookingData, bookingLocation: value })
									}
								>
									<>
										<option selected={true} disabled={true} value={'select'}>
										...
										{bookingLocation.map((location: any) => (
						...
						<Typography className="property-title">Booking Description</Typography>
						<Stack className="config-column">
							...
							<textarea
								...
								value={insertBookingData.bookingDesc}
								onChange={({ target: { value } }) => setInsertBookingData({ ...insertBookingData, bookingDesc: value })}
							></textarea>
						</Stack>
					</Stack>

					<Typography className="upload-title">Upload photos of your booking</Typography>
					<Stack className="images-box">
						...
						<Stack className="gallery-box">
							{insertBookingData?.bookingImages.map((image: string) => {
					...
					<Stack className="buttons-row">
						{router.query.bookingId ? (
							<Button className="next-button" disabled={doDisabledCheck()} onClick={updateBookingHandler}>
								<Typography className="next-button-text">Save</Typography>
							</Button>
						) : (
							<Button className="next-button" disabled={doDisabledCheck()} onClick={insertBookingHandler}>
								<Typography className="next-button-text">Save</Typography>
							</Button>
						)}
					</Stack>
